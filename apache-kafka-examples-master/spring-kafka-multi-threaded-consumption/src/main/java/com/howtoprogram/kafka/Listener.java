...
public class Listener {
	...
	@KafkaListener(id = "id0", topicPartitions = { @TopicPartition(topic = "SpringKafka", partitions = { "0" }) })
	public void listenPartition0(ConsumerRecord<?, ?> record) {
		System.out.println("Listener Id0, Thread ID: " + Thread.currentThread().getId());
		System.out.println("Listener Id0, Received: " + record);
		countDownLatch0.countDown();
	}

	@KafkaListener(id = "id1", topicPartitions = { @TopicPartition(topic = "SpringKafka", partitions = { "1" }) })
	public void listenPartition1(ConsumerRecord<?, ?> record) {
		System.out.println("Listener Id1, Thread ID: " + Thread.currentThread().getId());
		System.out.println("Listener Id1, Received: " + record);
		countDownLatch1.countDown();
	}

	@KafkaListener(id = "id2", topicPartitions = { @TopicPartition(topic = "SpringKafka", partitions = { "2" }) })
	public void listenPartition2(ConsumerRecord<?, ?> record) {
		System.out.println("Listener Id2, Thread ID: " + Thread.currentThread().getId());
		System.out.println("Listener Id2, Received: " + record);
